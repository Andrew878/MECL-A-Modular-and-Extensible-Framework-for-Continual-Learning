...
def test_synthetic_samples_versus_normal(original_task_datasetInterface, added_task_datasetInterface, PATH_MODELS,
                                         device='cuda', new_classes_per_increment=1, number_increments=5):
    ...
    num_per_cat_gen_class_test = 100
    ...
    for increment in range(0, number_increments):
        ...
        if (increment == 0):
            ...
            combined_task_branch_synthetic.load_existing_VAE(PATH_MODELS+"VAE MNIST pseudo epochs10,batch64,z_d50,synthFalse,rebuiltFalse,lr0.00035,betas(0.5, 0.999)lowest_error 93.99035051269531 increment0",False)
            ...
            list_categories_to_add = copy.deepcopy(added_task_datasetInterface.categories_list[0:new_class_index])
            list_categories_to_add_marginal = copy.deepcopy([added_task_datasetInterface.categories_list[new_class_index-1]])
            print("list_categories_to_add_marginal",list_categories_to_add_marginal)
            ...
            combined_task_branch_synthetic.create_blended_dataset_with_synthetic_samples(task_DIS_New_for_synthetic,
                                                                                         list_categories_to_add_marginal)
            ...
            test_generating_and_classification_ability_multi_tasks([combined_task_branch_no_synthetic,combined_task_branch_synthetic], number_per_category=num_per_cat_gen_class_test, device=device)
